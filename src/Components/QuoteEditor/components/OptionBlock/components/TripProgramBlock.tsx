...
export const TripProgramBlock: React.FC<TripProgramBlockProps> = ({
  ...
  return (
    <div className="tw-p-4 print:tw-p-2" data-component="trip-program">
      <div className={`tw-mb-4 tw-flex tw-flex-wrap tw-gap-2 tw-items-center ${printMode ? 'tw-hidden' : ''} print:tw-hidden`}>
        <div className="tw-flex tw-items-center tw-gap-2 tw-text-base tw-text-gray-600">
          <Filter size={18} />
          <span className="tw-font-semibold">Filtres:</span>
        </div>
        ...
        {STEP_FILTERS.map(filter => (
          <button
            ...
            disabled={readonly}
            className="tw-inline-flex tw-items-center tw-font-medium tw-rounded-full tw-border tw-transition-all tw-whitespace-nowrap tw-px-5 tw-py-2 tw-text-sm hover:tw-shadow-sm"
            style={{
